...
class Payfast extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    // 为新订单生成invoices的同时生成transaction
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        /** @var \Magento\Sales\Model\Order $order */
        $order = $payment->getOrder();
        $order_id = $order->getId();
        $transactionId = $payment->getAdditionalInformation('pf_payment_id');

        $relation_parent_real_id = $order->getRelationParentRealId();
        if ($order_id > 0 || !$relation_parent_real_id) {
            // 设置交易号，生成交易记录
    ...
    {
        $additionalInfomation = $payment->getAdditionalInformation('m_payment_id');
        // 退款处理
        ...
        if ($captureTxnId) {
            ...
            // 设置交易号，生成交易记录
            /** @var $payment \Magento\Sales\Model\Order\Payment */
            $payment->setTransactionId($additionalInfomation)
